<?php
include('auth.php');
require_superadmin(); // Only superadmins can access this page
...
if (isset($_GET['delete'])) {
    ...
    if ($id == $_SESSION['admin_id']) {
        ...
        $stmt->execute([$id]);
        header("Location: manage-headmasters.php?msg=deleted");
        exit();
    }
}
...
if (isset($_GET['toggle_status']) && isset($_GET['status'])) {
    ...
    if ($id == $_SESSION['admin_id']) {
        ...
        header("Location: manage-headmasters.php?msg=" . ($new_status == 'active' ? 'activated' : 'suspended'));
        exit();
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($role !== 'headmaster' && $role !== 'superadmin') {
        ...
        try {
            if (!empty($user_id)) {
                ...
                if (!empty($password)) {
                    ...
                    $stmt = $conn->prepare("UPDATE admins SET username = ?, full_name = ?, role = ?, password = ?, raw_password = ? WHERE id = ?");
                    $stmt->execute([$username, $full_name, $role, $hashed_password, $password, $user_id]);
                } else {
                ...
                if (empty($password)) {
                    ...
                    if ($stmt->fetch()) {
                        ...
                        $hashed_password = password_hash($password, PASSWORD_DEFAULT);
                        $stmt = $conn->prepare("INSERT INTO admins (username, full_name, password, raw_password, role) VALUES (?, ?, ?, ?, ?)");
                        $stmt->execute([$username, $full_name, $hashed_password, $password, $role]);
...
// Fetch all headmasters and superadmins
$stmt = $conn->prepare("SELECT * FROM admins WHERE role IN ('headmaster', 'superadmin') ORDER BY id DESC");
$stmt->execute();
$users = $stmt->fetchAll();
?>
<!DOCTYPE html>
        ...
        <div class="container-fluid px-0 px-md-4">
            ...
            <div class="page-header">
                <div class="d-flex align-items-center">
                    ...
                    <div>
                        <h1 class="mb-0">Headmaster Management</h1>
                        <p class="mb-0">Manage headmaster accounts for the system.</p>
                    </div>
                </div>
                <div class="header-actions">
                    <button class="btn btn-primary" data-bs-toggle="modal"
                        data-bs-target="#addUserModal">
                        <i class="fas fa-user-plus me-1"></i> Create Headmaster
                    </button>
                </div>
            </div>

            <!-- Toast Notifications -->
            <div class="toast-container position-fixed top-0 end-0 p-3 mt-5" style="z-index: 1055;">
                <?php if ($msg || isset($_GET['msg'])): ?>
                    <div class="toast align-items-center text-bg-success border-0 show" role="alert" aria-live="assertive" aria-atomic="true" style="border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
                        <div class="d-flex">
                            <div class="toast-body fw-medium" style="font-size: 0.95rem;">
                                ...
                                elseif (isset($_GET['msg']) && $_GET['msg'] == 'suspended')
                                    echo 'User suspended successfully!';
                                elseif (isset($_GET['msg']) && $_GET['msg'] == 'activated')
                ...
                <?php endif; ?>
                <?php if ($error): ?>
                    <div class="toast align-items-center text-bg-danger border-0 show" role="alert" aria-live="assertive" aria-atomic="true" style="border-radius: 12px; box-shadow: 0 10px 30px rgba(0,0,0,0.1);">
                        <div class="d-flex">
                            <div class="toast-body fw-medium" style="font-size: 0.95rem;">
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-3">
                                                <div class="user-avatar <?php echo $is_suspended ? 'avatar-suspended' : 'avatar-' . ($user['role'] ?? 'admin'); ?>" style="width: 40px; height: 40px; font-size: 0.9rem;">
                                                    <?php 
                                                        $name_for_avatar = !empty($user['full_name']) ? $user['full_name'] : $user['username'];
                                                        echo strtoupper(substr($name_for_avatar, 0, 2)); 
                                                    ?>
                                                </div>
                                                <div>
                                                     <div class="fw-bold" style="color: var(--prestige-text);">
                                                        <?php echo htmlspecialchars(!empty($user['full_name']) ? $user['full_name'] : $user['username']); ?>
                                                        <?php if ($is_suspended): ?>
            ...
            <div class="user-cards">
                <?php foreach ($users as $user): ?>
                    ...
                    $initials = strtoupper(substr(!empty($user['full_name']) ? $user['full_name'] : $user['username'], 0, 2));
                    ?>
                            ...
                            <div class="user-card-info">
                                 <div class="d-flex justify-content-between align-items-start">
                                    <div class="user-card-name" style="color: var(--prestige-text);">
                                        ...
                                        <?php endif; ?>
                                        <?php if (!empty($user['full_name'])): ?>
                                            <div class="text-muted small" style="font-size: 0.75rem; font-weight: normal; margin-top: 2px;">@<?php echo htmlspecialchars($user['username']); ?></div>
                                        <?php endif; ?>
                                    </div>
                ...
                <form method="POST" action="">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="full_name" id="edit_full_name" class="form-control" placeholder="e.g. Dr. John Doe" required>
                        </div>
                        <div class="mb-4">
