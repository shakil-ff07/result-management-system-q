...
$classes_data = $conn->query("SELECT id, class_name, section, academic_year FROM classes ORDER BY academic_year DESC, LENGTH(class_name), class_name ASC, section ASC")->fetchAll(PDO::FETCH_ASSOC);
$years = array_unique(array_column($classes_data, 'academic_year'));
rsort($years);
...
<head>
    ...
    <title>Official Registry Portal | SRMS</title>
...
<body>
    ...
    <nav class="prestige-nav-global">
        <div class="container d-flex justify-content-between align-items-center">
            ...
            <div class="nav-links-container">
                <a href="index.php" class="prestige-nav-link active">
    ...
    <header class="prestige-header">
        <div class="container">
            <span class="hero-badge">Student Result Management System</span>
            <h1 class="serif-font display-5 fw-bold mb-2">Central Registry</h1>
            <p class="opacity-75 small">Authenticating and retrieving official academic records.</p>
        </div>
    </header>
    ...
    <div class="registry-container">
        <div class="registry-card">
            <div class="registry-header">
                <h2 class="serif-font">Registry Lookup</h2>
                <p class="text-muted small mb-0">Enter student credentials to access the secure database.</p>
            </div>

            <form action="result.php" method="GET" class="registry-body">
                <div class="row g-4">
                    <!-- Roll Number -->
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="custom-select-wrapper">
                            <select name="year" id="year" class="native-select" required>
                            ...
                            <div class="custom-options">
                                <div class="custom-option" data-value="">Select Session</div>
                                <?php foreach ($years as $y): ?>
                                    <div class="custom-option" data-value="<?php echo $y; ?>"><?php echo $y; ?></div>
                                <?php endforeach; ?>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="custom-select-wrapper">
                            <select name="class_name" id="class_name" class="native-select" required disabled>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="custom-select-wrapper">
                            <select name="exam_type" class="native-select" required>
                            ...
                            <div class="custom-options">
                                <div class="custom-option" data-value="">Select Cycle</div>
                                <div class="custom-option" data-value="Half Yearly">Half Yearly</div>
                                <div class="custom-option" data-value="Final">Final Examination</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-12">
                        <button type="submit" class="btn-retrieve">
                            <i class="fas fa-shield-alt"></i> RETRIEVE ACADEMIC RECORD
                        </button>
                    </div>
                </div>
            </form>
    ...
    <script>
        ...
        yearSelect.addEventListener('change', function () {
            ...
            const classWrapper = classSelect.closest('.custom-select-wrapper');
            const classTrigger = classWrapper.querySelector('.custom-select-trigger');
            const classOptionsContainer = classWrapper.querySelector('.custom-options');
            const classTriggerText = classTrigger.querySelector('span');

            classSelect.value = "";
            classTriggerText.textContent = "Select Level";
            sectionSelect.value = "";
            sectionSelect.closest('.custom-select-wrapper').querySelector('.custom-select-trigger span').textContent = "Select Section";

            classTrigger.classList.add('disabled');
            classSelect.disabled = true;
            sectionSelect.closest('.custom-select-wrapper').querySelector('.custom-select-trigger').classList.add('disabled');
            sectionSelect.disabled = true;

            if (!selectedYear) return;

            const validClasses = [...new Set(allData.filter(item => item.academic_year == selectedYear).map(item => item.class_name))].sort((a, b) => a.length - b.length || a.localeCompare(b));
            let nativeHtml = '<option value="">Select Level</option>';
            let customHtml = '<div class="custom-option" data-value="">Select Level</div>';
            validClasses.forEach(cls => {
                nativeHtml += `<option value="${cls}">${cls}</option>`;
                customHtml += `<div class="custom-option" data-value="${cls}">${cls}</div>`;
            });
            classSelect.innerHTML = nativeHtml;
            classOptionsContainer.innerHTML = customHtml;
        ...
        classSelect.addEventListener('change', function () {
            ...
            const validSections = allData.filter(item => item.academic_year == selectedYear && item.class_name == selectedClass).map(item => item.section).sort();
